...
class Rotate:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown() and rospy.Time.now() - start_time < rospy.Duration(8.0):
            self.msg_vel.linear.x= 0
...
class map_navigation:
    ...
    #     self.current_yaw = yaw -(math.pi/2)
    #     self.current_x = msg.pose.pose.position.x
    #     self.current_y = msg.pose.pose.position.y

    # #get x and y of pillar or hydrant
    # def perception_cb(self,msg):
    #     if msg.objects_detected:
    #         for i in msg.objectOfInterest:
    #             if i.distance < self.smallest_d  and (i.object_type==2 or i.object_type==3):
    #                 self.smallest_d = i.distance
    #                 self.angle = i.angle
    #                 #print(self.smallest_d)
    #                 self.find_pose()

    # def find_pose(self):
    #     #quadrant1
    #     print(self.current_yaw)
    #     if (self.current_yaw+self.angle) < (math.pi/2):
    #         z = math.pi-self.current_yaw-self.angle
    #         x = self.smallest_d * math.sin(z)
    #         y = self.smallest_d * math.cos(z)
    #         pose_x = x + self.current_x
    #         pose_y = y + self.current_y
    #         print("wtf")
    #         print("x is: {}. y is: {}".format(pose_x,pose_y))
    #     self.smallest_d = 6
